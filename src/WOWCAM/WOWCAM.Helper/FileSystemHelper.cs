...
public sealed class FileSystemHelper
{
    ...
    public static Version GetExecutableFileVersion(string filePath)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(filePath);

        filePath = Path.GetFullPath(filePath);

        if (OperatingSystem.IsWindows() && Path.GetExtension(filePath) != ".exe")
        {
            throw new InvalidOperationException("The given executable file path is not a file which has the \".exe\" file extension.");
        }

        var fileVersionInfo = FileVersionInfo.GetVersionInfo(filePath);
        var productVersion = fileVersionInfo.ProductVersion ?? throw new InvalidOperationException("Could not determine product version of given executable file.");
