...
echo.
echo Will perform the following 4 steps now:
echo.
echo 1) clean projects
echo 2) dotnet publish
echo 3) copy 2 executables
...
mkdir .\%RELEASE%
copy /B /V /Y .\src\WOWCAM\WOWCAM.Windows\bin\Release\net10.0-windows\win-x64\publish\WOWCAM.Windows.exe .\%RELEASE%\wowcamui.exe >NUL
copy /B /V /Y .\src\WOWCAM\WOWCAM\bin\Release\net10.0\win-x64\publish\WOWCAM.exe .\%RELEASE%\wowcam.exe >NUL
copy /B /V /Y .\wowcam.xml.sample .\%RELEASE% >NUL

echo Finished (you can now deploy everything inside the '%RELEASE%' folder)
echo.
